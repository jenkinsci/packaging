...
read_old_options() {
	if [ -n "${JENKINS_ARGS}" ]; then
		if [ -n "${NAME}" ]; then
			...
			TMP_JENKINS_WEBROOT=$(printf '%s' "${JENKINS_ARGS}" | sed -n 's/.*--webroot=\([[:alnum:][:punct:]]*\).*/\1/p')
			TMP_JENKINS_LOG=$(printf '%s' "${JENKINS_ARGS}" | sed -n 's/.*--logfile=\([[:alnum:][:punct:]]*\).*/\1/p')
			...
			TMP_JENKINS_HTTPS_KEYSTORE=$(printf '%s' "${JENKINS_ARGS}" | sed -n 's/.*--httpsKeyStore=\([[:alnum:][:punct:]]*\).*/\1/p')
			TMP_JENKINS_HTTPS_KEYSTORE_PASSWORD=$(printf '%s' "${JENKINS_ARGS}" | sed -n 's/.*--httpsKeyStorePassword=\([[:alnum:][:punct:]]*\).*/\1/p')
			...
			TMP_JENKINS_EXTRA_LIB_FOLDER=$(printf '%s' "${JENKINS_ARGS}" | sed -n 's/.*--extraLibFolder=\([[:alnum:][:punct:]]*\).*/\1/p')
			TMP_PREFIX=$(printf '%s' "${JENKINS_ARGS}" | sed -n 's/.*--prefix=\([[:alnum:][:punct:]]*\).*/\1/p')
